...
CREATE TABLE inventario_movimientos (
  ...
  documento_tipo TEXT,
  lote_pistoleo_id UUID REFERENCES pistoleo_batches(id) ON DELETE SET NULL,
  observaciones TEXT,
...
BEGIN
  CASE NEW.tipo
    WHEN 'entrada' THEN stock_delta := NEW.cantidad;
    WHEN 'salida' THEN stock_delta := -NEW.cantidad;
    WHEN 'ajuste_positivo' THEN stock_delta := NEW.cantidad;
    WHEN 'ajuste_negativo' THEN stock_delta := -NEW.cantidad;
    WHEN 'transferencia_origen' THEN stock_delta := -NEW.cantidad;
    WHEN 'transferencia_destino' THEN stock_delta := NEW.cantidad;
    WHEN 'devolucion_proveedor' THEN stock_delta := -NEW.cantidad;
    WHEN 'inventario_inicial' THEN stock_delta := NEW.cantidad;
    ELSE stock_delta := 0;
  END CASE;
...
-- ============================================================
-- FUNCIÓN PARA OBTENER STOCK POR UBICACIÓN (RECURSIVA)
-- ============================================================
